...
func ParseCSV(src io.Reader) ([]Ownership, error) {
	...
	for {
		...
		if e != nil {
			return nil, fmt.Errorf("файл повреждён: %w", e)
		}
		line, _ := reader.FieldPos(0)
		for _, value := range row {
			if !utf8.ValidString(value) {
				return nil, fmt.Errorf("строка %d: файл должен быть в кодировке UTF-8", line)
			}
		}
		source := map[string]string{}
		values := map[string]string{}
		for key, i := range columns {
			source[key] = row[i]
			values[key] = clean(row[i])
		}
		if strings.ContainsAny(strings.TrimSpace(source["Правообладатель (правообладатели)"]), "\r\n") {
			return nil, fmt.Errorf("строка %d: несколько собственников в одной ячейке — укажите каждого отдельной строкой", line)
		}
		if values["№ помещения"] == "" {
			if previous == nil {
				return nil, fmt.Errorf("строка %d: не указан номер помещения", line)
			}
			for _, key := range premisesKeys {
				value := values[key]
				if value != "" && value != previous[key] {
					return nil, fmt.Errorf("строка %d: столбец «%s» расходится со строкой выше — для нового помещения укажите его номер", line, key)
				}
		...
		for _, h := range requiredColumns {
			if get(h) == "" {
				return nil, fmt.Errorf("строка %d: не заполнен столбец «%s»", line, h)
			}
		}
		owned, e := decimal(get("Долевая площадь, м²"))
		if e != nil {
			return nil, fmt.Errorf("строка %d: долевая площадь: %w", line, e)
		}
		total, e := decimal(get("Общая площадь, м²"))
		if e != nil {
			return nil, fmt.Errorf("строка %d: общая площадь: %w", line, e)
		}
		if compareDecimals(owned, total) > 0 {
			return nil, fmt.Errorf("строка %d: долевая площадь больше площади помещения", line)
		}
		address := get("Адрес объекта")
		if loc := premisesSuffix.FindStringIndex(address); loc != nil {
			address = strings.TrimSpace(address[:loc[0]])
		}
		if address == "" {
			return nil, fmt.Errorf("строка %d: не указан адрес дома", line)
		}
	...
	if len(result) == 0 {
		return nil, fmt.Errorf("в реестре нет ни одного собственника")
	}
	return result, nil
}
